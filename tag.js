...
let requestCount = 1; // Counter for the file names

// Klasörün var olup olmadığını kontrol et, yoksa oluştur
const folderPath = path.join(__dirname, 'tags');
...
const fetchData = async () => {
  try {
    const response = await axios.get('https://api.arkm.com/tag/top', {
      params: {
        tag: 'dex-aggregator',
        page: 1,
      },
      headers: {
        ...
        'x-payload': '896eb8cbe86c87b6a99f61eb1992571dd4af55613604512ad353b6cc57e59f31',
        'x-timestamp': '1731159788',
      }
    });

    // Dosya adını ve yolu belirle (tags klasörüne kaydediyoruz)
    const fileName = path.join(folderPath, `data_tg_${requestCount}.json`);
    fs.writeFileSync(fileName, JSON.stringify(response.data, null, 2), 'utf-8');
    console.log(`Data has been saved to ${fileName}`);

    requestCount++; // Counter'ı bir sonraki dosya için artır
